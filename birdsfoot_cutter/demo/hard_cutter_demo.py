...
def main():
	birdsfoot = r'C:\Users\thetk\Documents\BYU\Work\pythonProject\birdsfoot_cutter\mesh01.stl'
	#birdsfoot = r'C:\Users\thetk\Documents\BYU\Work\pythonProject\birdsfoot_cutter\mesh23.stl'
	# Make the point cloud (which can be read as a NumPy array)
	mesh_object = pv.read(birdsfoot)
	mesh_points = mesh_object.points
	mesh_faces = mesh_object.regular_faces
	print(mesh_faces)

	#demo_mesh = pv.PolyData(verts=mesh_points, faces=mesh_faces)
	#demo_mesh.save('demo_mesh.stl')
	...
	# Generate two random orthonormal vectors from the normal of the plane after outliers are removed
	b1, b2 = generate_orthonormal_basis(full_plane, seeded=1)
	b3 = full_plane.normal.unit()
	
	
	# Transform into 2D once at the start, then make our transformations within 2D
	trans_proj_cloud = sktrf.transform_coordinates(full_points, full_plane.point, (b1, b2, b3))[:, 0:2]
	# This will only look at 180 degrees, taking mins and maxs to evaluate two edges per rotation
	# TODO: Make this a function that operates on iteratively smaller intervals to get a more specific bottom edge alignment quickly
	# Apply rotations from zero to 2*pi to our 1st and 2nd bases

	# Look at this many rotational positions to roughly find the bottom edge
	rough_rot_steps = 12
	rough_resolution = np.pi/rough_rot_steps
	# Making a full rotation, this finds the bottom edge standard deviation for each step
	rough_rot_std = rotate_solve_flatness(trans_proj_cloud, rot_steps=rough_rot_steps, verification_percentile=0.7)
	print(rough_rot_std)
	...
	# This value will be high if this is the case.
	...
	# and is the right method to use if two corners are significantly rougher than the other two.
	...
	elif max_valley_confidence < absolute_min_confidence > local_min_confidence:
		# In this case, our bottom edge
		bottom_side_index = clean_side_index
	elif local_min_confidence < max_valley_confidence > absolute_min_confidence:
	...
	bottom_rotation = R.from_rotvec(fine_angle_to_bottom * b3)
	...
	rough_hinge_offset_45deg = rough_hinge_offset * np.sqrt(2)
	...
	y_map_lower = (final_cloud[:, 1] > hinge_offset) & (final_cloud[:, 1] < 70-hinge_offset)
	...
	lower_left_verts = skobj.Points(final_cloud[ll_map])
	lower_right_verts = skobj.Points(final_cloud[lr_map])
	upper_left_verts = skobj.Points(final_cloud[ul_map])
	upper_right_verts = skobj.Points(final_cloud[ur_map])
	...
	# print(np.max(lower_left_faces))
	# print(mesh_points.shape)
	# print(np.all(np.isin(lower_left_faces, indicies_clean_aligned_final[ll_map])))
	# print(lower_left_faces.shape)
	# # This below is the format that pyvista supposedly needs to make .stl files
	# lower_left_faces = np.hstack([np.full((lower_left_faces.shape[0], 1), 3), lower_left_faces])
	# print(lower_left_faces.shape)
	# print(lower_left_faces)
	
	# I could not get this to work for whatever reason
	#lower_left_panel_mesh = pv.PolyData(verts=mesh_points, faces=lower_left_faces).clean(inplace=True)
	#lower_right_panel_mesh = pv.PolyData(verts=lower_right_verts, faces=lower_right_faces)
	#upper_left_panel_mesh = pv.PolyData(verts=upper_left_verts, faces=upper_left_faces)
	#upper_right_panel_mesh = pv.PolyData(verts=upper_right_verts, faces=upper_right_faces)
	#lower_left_panel_mesh.save('ll.stl')
	#lower_right_panel_mesh.save('lr.stl')
	#upper_left_panel_mesh.save('ul.stl')
	#upper_right_panel_mesh.save('ur.stl')
	...
	save_mesh(lower_left_faces, 'll.stl')
	save_mesh(lower_right_faces, 'lr.stl')
	save_mesh(upper_left_faces, 'ul.stl')
	save_mesh(upper_right_faces, 'ur.stl')
	...
	#skobj.Points(aligned_cloud[plane_angle_map & plane_y_map]).plot_3d(ax, s=0.3, alpha=0.1, c='c')
	#skobj.Points(aligned_cloud).plot_3d(ax, s=0.03, alpha=0.01, c='c')
	#skobj.Points(aligned_cloud[orientation_check_map]).plot_3d(ax, s=0.03, alpha=0.01, c='c')
	
	# try:
	# 	skobj.Points(aligned_cloud[orientation_check_map][orientation_check_points[:, 2] > scan_offset]).plot_3d(ax, s=0.3, alpha=0.1, c='c')
	# except:
	# 	print("No points are above scan offset")
	# try:
	# 	skobj.Points(aligned_cloud[orientation_check_map][orientation_check_points[:, 2] < -scan_offset]).plot_3d(ax, s=0.3, alpha=0.1, c='m')
	# except:
	# 	print("No points are below scan offset")
	
	#skobj.Points(bot_left_corner_points).plot_3d(ax, s=0.03, alpha=0.01, c='c')
	#bot_left_corner.plot_3d(ax, s=1, alpha=1, c='k')
	#skobj.Points(final_cloud).plot_3d(ax, s=0.03, alpha=0.01, c='c')
	
	lower_left_verts.plot_3d(ax, s=0.03, alpha=0.1, c='c')
	lower_right_verts.plot_3d(ax, s=0.03, alpha=0.1, c='c')
	upper_left_verts.plot_3d(ax, s=0.03, alpha=0.1, c='c')
	upper_right_verts.plot_3d(ax, s=0.03, alpha=0.1, c='c')
...
def rotate_solve_flatness(point_cloud_2d, rot_steps, start=0.0, stop=2*np.pi, linear_steps=100, verification_percentile=0.5):
	...
	for step_i, angle in [(step, -(start+(stop-start)*step/rot_steps)) for step in iter_range]:
		...
		# TODO: Find an implementation for standard devation that caters to many points close together and isn't disturbed
		...
		print("std: ", rot_std[step_i])
		if save_max:
			rot_std[step_i+floor(rot_steps/2)] = np.std(y_maximums[start:end])
			print("std + 180 deg: ", rot_std[step_i+floor(rot_steps/2)])
		#plt.clf()
